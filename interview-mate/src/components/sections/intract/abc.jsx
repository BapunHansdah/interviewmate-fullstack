...
//changing the date in slot
  ...
  const openModal = (time,by,id,price,duration,topic,booked) => {
    setPaymentInfo({time,by,id,price,duration,topic,booked})
    setOpen(true);
  }
...
const bg = {
  ...
  async function scheduleSlot(id,by,booked){

      if(topicSelected.length===0){
         swal.fire("select atleast one topic")
         return ;
      }
       try{

             await axios.put(`/api/slot/schedule/${id}`,{by:by,booked:booked,topic:topicSelected},{
                'headers':{
            ...
            <div className="flex flex-wrap  gap-2">      
               {
                 availSlot ? availSlot.map((s,i)=>{
                  if(s.date === moment(date).format("DD MMM YYYY")){
                    return (
                          <div key={s._id} onClick={()=>openModal(s.time,s.by,s._id,s.price,s.duration,topic,s.booked)} className={`text-xs border border-black text-white hover:bg-opacity-90 rounded cursor-pointer  flex justify-between`}>
            ...
            <div className="">
                <Modal isOpen={open} style={bg} className="max-w-md mx-auto border border-black bg-white h-fit mt-40">
                  <div className="p-2 flex justify-center items-center">
                    ...
                    <div><button className="bg-green-500 text-white px-4 py-1 rounded" onClick={()=>scheduleSlot(paymentInfo.id,paymentInfo.by,paymentInfo.booked)}>Pay</button></div>
